...
export function SystemSettings() {
  ...
  const fetchSettings = async () => {
    try {
      const response = await fetch('http://116.50.227.178:3001/api/system-settings', {
    ...
    try {
      ...
      const usersWithHours = await Promise.all(
        usersData.map(async (user: any) => {
          try {
            const hoursResponse = await fetch(`http://116.50.227.178:3001/api/user-hours-progress`, {
              headers: { 
  ...
  const handleHoursAdjustment = async () => {
    ...
    setSaving(true);
    try {
      const response = await fetch(`http://116.50.227.178:3001/api/users/${selectedUser.id}/hours-adjustment`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
